...
> 这是 [yc-duan/fastctx](https://github.com/yc-duan/fastctx) 的 Fooljack Windows
> 稳定性增强分支，已同步到上游 `v0.2.4`
...
`read` 响应使用与宿主一致的精确 `o200k_base` 计算收口，动态续读或完成说明也计入 token 预算。文本读取采用流式收集；正文达到有界预算后停止继续保存，同时保留 1 基行号、行截断、编码说明和精确的下一次 offset。批量读取最多接受 32 个成员，保持请求顺序，并返回可原样重放的逐文件续读数组。hex 通道默认使用 2,000 行窗口，先建立一次 prefix checkpoint 图，再只探测每个候选的短终态说明，最后独立完整校验一次 token 数。这样不会为每个被拒绝的 prefix 重新拼接并完整分词整个 hex 响应，同时不改变 hex 字节、列格式、offset、limit 或 `Partial`/`Complete` 语法。

响应预算是安全边界，不保证任意输入都能一次返回。响应过大时，应提高对应预算，或按返回的续读参数继续调用。文本和 shell 展示行最多 2,000 个字符；grep 搜索缓冲和捕获匹配内容各有 64 MiB 安全上限；replace 输入文件和结果默认限制为 256 MiB，可配置为 64 MiB 至 4 GiB；遍历、候选文件、PDF 与协议上限仍然有效。这些限制与精确 token 收口相互独立。

优化后的 replace 路径会识别不含 capture 引用的字面量替换，增量扫描匹配，并在每个文件内复用一次编码后的替换字节。含 capture 的替换仍保留 `$0`、`$1`、`${name}`、零宽匹配、`max_replacements`、编码和结果大小限制语义。所有写入仍使用同目录原子提升和 revision/CAS 检查，保留编码/BOM/EOL、尾随换行及未修改字节，保持符号链接目标语义，并拒绝硬链接目标。

本轮测量使用真实 MCP `serve` 协议和 release 二进制。在一台 Windows 机器上，64,000,000 字节二进制（4,000,000 行 hex）在 checkpoint 路径前的预算 100 和 1,000 中位数约为 344 ms 和 35.5 s，之后约为 5.4 ms 和 46.8 ms；预算 8,500 的样本约为 398 ms。另一组 70 MiB 文本 fixture 在预算 100–8,500 间约为 55–59 ms，预算 8,500 下完整 32 文件批次约为 21 ms。这些数字只表示可复现的测量范围，不是普遍 Bash 持平声明、与机器无关的阈值，也不保证所有文件系统、编码或负载都相同。除非调用方另行启用并验证确定性条件，否则并发运行不承诺逐位一致。
