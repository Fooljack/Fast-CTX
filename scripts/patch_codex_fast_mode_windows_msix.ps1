...
function Test-CodexAppPath {
  ...
  return (
    ...
    (-not [string]::IsNullOrWhiteSpace((Get-AppExecutablePath (Split-Path -Parent $app)))) -and
...
function Write-PatcherFiles {
  ...
  $gpt56ModelUiPatcherPath = Join-Path $WorkDir 'PatchGpt56ModelUi.cjs'
...
'@
  ...
  return [pscustomobject]@{
    ...
    Gpt56ModelUi = $gpt56ModelUiPatcherPath
  }
}
...
function Find-PatchTargets {
    ...
    foreach ($candidate in $desktopFeatureMainCandidates) {
      ...
      if ($text.Contains('CODEX_ELECTRON_ENABLE_WINDOWS_COMPUTER_USE') -and
          (($text -match '([A-Za-z_$][\w$]*)=([A-Za-z_$][\w$]*)===`win32`&&([A-Za-z_$][\w$]*)\.CODEX_ELECTRON_ENABLE_WINDOWS_COMPUTER_USE===`1`\?\{\.\.\.([A-Za-z_$][\w$]*),computerUse:!0,computerUseNodeRepl:!0\}:\4') -or
           ($text -match 'function [A-Za-z_$][\w$]*\(e,\{buildFlavor:[^}]+,env:[^}]+,platform:[^}]+\}=\{\}\)\{let [A-Za-z_$][\w$]*=[A-Za-z_$][\w$]*===`win32`&&e\.computerUse===!0\?') -or
  ...
  if ([string]::IsNullOrWhiteSpace($localeI18nTarget)) {
    ...
    Fail 'could not find GPT-5.6 model UI patch target in extracted assets'
  }
  ...
  Write-Log "GPT-5.6 model UI patch target: $gpt56ModelUiTarget"
  ...
  return [pscustomobject]@{
    ...
    Gpt56ModelUi = $gpt56ModelUiTarget
...
function Invoke-PatchAppAsar {
  ...
  $gpt56ModelUi = Invoke-NodePatcher $nodePath $patchers.Gpt56ModelUi @($targets.Gpt56ModelUi)
  Write-Log "GPT-5.6 model UI patch result: $gpt56ModelUi"
  $pluginArgs = @(
  ...
  if ($fast -eq 'already-patched' -and
      ...
      $gpt56ModelUi -eq 'already-patched' -and
