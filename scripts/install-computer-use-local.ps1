...
function Get-CurrentCodexAppServerRuntimeInventory {
  ...
  if ($codexCandidates.Count -eq 0) {
    throw "no current user-local Codex CLI matches the installed package under $localBinRoot; run the normal Computer Use repair before strict verification"
  }
  if ($cuaCandidates.Count -eq 0) {
    throw "no current user-local CUA Node runtime matches the installed package under $localCuaRoot; run the normal Computer Use repair before strict verification"
  }
