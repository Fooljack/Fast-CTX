...
$fakeRegSourcePath = Join-Path $fakeBinRoot 'reg.cs'
...
[System.IO.File]::WriteAllBytes(
  (Join-Path $scriptsRoot 'installManifest.mjs'),
  [System.IO.File]::ReadAllBytes($officialInstallerPath)
)
[System.IO.File]::WriteAllBytes(
  (Join-Path $scriptsRoot 'extension-ids.json'),
  [System.IO.File]::ReadAllBytes($officialExtensionIdsPath)
)
...
if ($PSVersionTable.PSEdition -eq 'Core') {
  ...
  if (-not $csc) {
    throw 'PowerShell 7 regression requires the Windows .NET Framework C# compiler'
  }
  [System.IO.File]::WriteAllText($fakeRegSourcePath, $fakeRegSource, [System.Text.UTF8Encoding]::new($false))
  $compilerOutput = @(& $csc /nologo /target:exe "/out:$fakeRegPath" $fakeRegSourcePath 2>&1)
  if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $fakeRegPath -PathType Leaf)) {
    throw "Failed to compile isolated reg.exe fixture: $($compilerOutput -join [Environment]::NewLine)"
  }
} else {
...
try {
  ...
  Write-Output 'Chrome native-host official-installer origin regression passed'
} finally {
