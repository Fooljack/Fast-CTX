...
$repairedLocalCodexRoot = Join-Path $inventoryFixture 'repaired-local-codex'
$staleManagedBin = Join-Path $repairedLocalCodexRoot 'bin\package-stale'
$staleManagedCua = Join-Path $repairedLocalCodexRoot 'runtimes\cua_node\package-stale'
New-Item -ItemType Directory -Force -Path $staleManagedBin, $staleManagedCua | Out-Null
Set-Content -LiteralPath (Join-Path $staleManagedBin '.fooljack-managed-runtime') -Value 'stale' -Encoding ASCII
Set-Content -LiteralPath (Join-Path $staleManagedCua '.fooljack-managed-runtime') -Value 'stale' -Encoding ASCII
$repairedInventory = Get-CurrentCodexAppServerRuntimeInventory `
  -PackageResourcesRoot $packageResourcesRoot -LocalCodexRoot $repairedLocalCodexRoot -AllowRepair
foreach ($runtimePath in @($repairedInventory.CodexCliPath, $repairedInventory.NodePath, $repairedInventory.NodeReplPath)) {
  if (-not $runtimePath.StartsWith($repairedLocalCodexRoot, [StringComparison]::OrdinalIgnoreCase)) {
    throw "Runtime repair selected a path outside the user-local root: $runtimePath"
  }
}
foreach ($marker in @(
  (Join-Path (Split-Path -Parent $repairedInventory.CodexCliPath) '.fooljack-managed-runtime'),
  (Join-Path (Split-Path -Parent (Split-Path -Parent $repairedInventory.NodePath)) '.fooljack-managed-runtime')
)) {
  if (-not (Test-Path -LiteralPath $marker -PathType Leaf)) {
    throw "Runtime repair did not write its ownership marker: $marker"
  }
}
if ((Test-Path -LiteralPath $staleManagedBin) -or (Test-Path -LiteralPath $staleManagedCua)) {
  throw 'Runtime repair did not remove stale Fooljack-managed runtime directories'
}
...
if ($discoveredInventory.CodexCliPath -match '(?i)[\\/]package-resources[\\/]' -or
  ...
  Write-Output 'Chrome app-server config regression passed'
} finally {
