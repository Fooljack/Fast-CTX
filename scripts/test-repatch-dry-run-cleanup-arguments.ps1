...
$wrapperText = [System.IO.File]::ReadAllText($wrapper, [System.Text.UTF8Encoding]::new($false))
foreach ($requiredFinalizerGuardMarker in @(
  ...
  if (-not $wrapperText.Contains($requiredFinalizerGuardMarker)) {
    throw "repatch wrapper is missing finalizer crash guard marker: $requiredFinalizerGuardMarker"
  }
}
...
Assert-KeySet $keepCapture `
  ...
  Write-Output 'Repatch DryRun cleanup argument forwarding regression passed'
} finally {
